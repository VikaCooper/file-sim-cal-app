...
import { Card, Alert, Row, Col, Form, Input, InputNumber, Upload } from 'td-ui';
import { Link, withRouter } from 'react-router-dom';
import '../styles/quicklearn.css';
...
class QuickLearn extends Component {
    ...
    render(){
        const alertContent=<div className={'line-ctr'}>
            <p>文件个数：文件个数即为您所要比对的文件个数，不得小于两个。</p>
            <p>文件上传：上传您打包好的所有要比对的文档，支持格式：zip，rar，gzip。</p>
        </div>;
        return <div>
            <Row>
                <header style={{fontWeight: 'bold', fontSize: '1.3em'}}>快速上手</header>
                ...
                <Alert type={'info'} message={alertContent} />
            </Row>
        </div>;
    }
}
